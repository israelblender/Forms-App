...
class Dragged:
    ...
    def PlaceOnCanvas(self,Canvas,XY):
        """
        Chame esse método para nos colocar em uma tela arbitrária no local XY.
        
        Se já estamos em uma tela, então nos removemos dessa tela
             antes de nos instalar na tela especificada. Provavelmente não melhor
             para chamar esse método enquanto estamos no meio do arrasto.
            
        Este método é útil se você quiser colocar um objeto arrastável recém-cunhado
             diretamente em uma tela sem que o usuário tenha que arrastá-lo para lá. isto
             também funcionará se você simplesmente quiser mover um objeto arrastável existente
             de uma tela para outra.
        """
        # se estivermos em uma tela existente, saia dela.
        self.Vanish()
        #Desenhe nosso proprio canvas.
        #desenhe-se na nova tela. Como estamos sendo colocados arbitrariamente, 
        #zeramos o deslocamento.
        self.OffsetX = 0
        self.OffsetY = 0
        self.Appear(Canvas,XY)
        #ligar ao ButtonPress para que o usuário possa arrasta-lo
        self.Label.bind('<ButtonPress>',self.Press)
        
    def dnd_end(self,Target,Event):
        """
        Isso é chamado quando somos descartados.
        """
        Blab(1, "%s has been dropped; Target=%s"%(self.Name,`Target`))
        if self.Canvas==None and self.OriginalCanvas==None:
            # Fomos criados e depois abandonados no meio do nada, 
            #ou nos disseram para nos autodestruir. Em ambos os casos, 
            #nada precisa ser feito e vamos evaporar em breve.
            return
        if self.Canvas==None and self.OriginalCanvas<>None:
            # Nos viviamos anteriormente no OriginalCanvas e o usuario arrastou e nos deixou no meio do nada. O que voce faz
            #aqui depende do seu gosto pessoal. Existem 2 opcoes:
            # 1) Nao faca nada. O objeto arrastado ira simplesmente evaporar. De fato
            # você esta dizendo "soltando um objeto existente no meio
            # do nada a exclui ". Pessoalmente, nao gosto dessa opcao porque
            # significa que, se o usuario, enquanto arrasta um objeto importante,
            # contorce o dedo do mouse enquanto o objeto esta no meio
            # em nenhum lugar o objeto e imediatamente excluido. Oops
            # 2) Ressuscitar o rotulo original (que esteve la, mas invisivel)
            # dizendo assim "soltando um objeto arrastado existente no meio de
            # nenhum lugar e como se nenhum arrasto tivesse ocorrido ". 
            # Isso e o que o codigo que segue faz.
